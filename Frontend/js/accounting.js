...
function switchAccountingTab(view) {
    ...
    // Update Nav
    document.querySelectorAll('#accounting .nav-link').forEach(link => link.classList.remove('active'));
...
async function loadInvoices() {
    ...
    try {
        const response = await authenticatedFetch('/accounting/invoices');
...
function displayInvoices(invoices) {
    ...
    if (invoices.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="5" class="text-center">No invoices found. Create your first invoice!</td></tr>';
        return;
    }

    tableBody.innerHTML = invoices.map(invoice => {
        const patientName = invoice.patientId ? `${invoice.patientId.firstName} ${invoice.patientId.lastName}` : 'Unknown Patient';
...
async function loadExpenses() {
    ...
    try {
        const response = await authenticatedFetch('/accounting/expenses');
        const data = await response.json();
...
async function loadPatientsList() {
    try {
        const response = await authenticatedFetch('/patients');
        const data = await response.json();
        if (data.success) {
...
function showCreateInvoiceModal() {
    ...
    const patientName = prompt("Select Patient (Enter ID or Name for demo, usually a dropdown):", patientsList[0].firstName);
